...
public class Groovy implements DiscordCommandInterface {
    ...
    public void playVideo(MessageReceivedEvent message, List<String> args) {
        ...
        if (guildMusicManager == null) {
            ...
            AudioManager voiceChat = confirmedChannel.getGuild().getAudioManager();
            AudioPlayerSendHandler audioPlayerSendHandler = new AudioPlayerSendHandler(audioPlayer);
            guildMusicManager = new GuildMusicManager(audioPlayer, message.getTextChannel(), voiceChat);

            //Tell audioPlayer to notify musicManager of events happening
            audioPlayer.addListener(guildMusicManager);
            voiceChat.setSendingHandler(audioPlayerSendHandler);
            voiceChat.openAudioConnection(confirmedChannel);
            guildMusic.put(message.getGuild().getName(), guildMusicManager);
        }
    ...
    @DiscordCommand(name = "!status", numberOfArgs = "0", help = "!die (kills the music)")
    public void status(MessageReceivedEvent message, List<String> args) {
        ...
        message.getChannel().sendMessage(String.valueOf(manager.isPlaying())).queue();
